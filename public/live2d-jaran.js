...
`
  ...
  // ]
  // 本文件由 next/script 注入，src 形如 `/space/live2d-jaran.js`。
  // 从中推导静态资源前缀，兼容部署在子路径（GitHub Pages /space/）的情况。
  const LIVE2D_BASE = (() => {
    const script = document.querySelector('script[src*="live2d-jaran.js"]')
    const src = script ? script.getAttribute('src') || '' : ''
    return src.replace(/live2d-jaran\.js.*$/, '')
  })()
